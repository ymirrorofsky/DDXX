angular.module('starter.services', [])
	...
	.factory('Mc', function($resource, $rootScope, $ionicLoading, ENV, Message, $q, $state, Storage) {
		...
		return {
			...
			getApply: function() {
				var res = $resource(ENV.YD_URL, { do: 'users', op: '@op' });
				//				console.log($rootScope.globalInfo.user.isShop);
				var deferred = $q.defer();
				Message.loading();
				res.save({ op: 'applyfor' }, function(response) {
					console.log(response);
					Message.hidden();
					if(response.code == 0) {
						deferred.resolve(response.data);
	...
	.factory('Apply', function($resource, $ionicLoading, ENV, Message, $q, $state, $rootScope, Storage) {
		...
		return {
			...
			getApplyCA: function() {
				var deferred = $q.defer();
				Message.loading();
				resource.save({ op: 'applyforca' }, function(response) {
					console.log(response);
					Message.hidden();
					if(response.code == 0) {
						deferred.resolve(response.data);
			...
			//申请业务员，检查市级代理人是否存在
			checkPAgent: function(pAgentNum) {
				var deferred = $q.defer();
				Message.loading();
				resource.save({ op: 'applyforca', type: 'check', pAgentNum: pAgentNum }, function(response) {
					if(response.code == 0) {
						deferred.resolve(response.data);
					} else if(response.code == 1) {
						Message.show(response.msg);
			...
			applyCAgent: function(pAgentNum) {
				var deferred = $q.defer();
				Message.loading();
				resource.save({ op: 'applyforca', type: 'save', pAgentNum: pAgentNum }, function(response) {
			...
			getApplySM: function() {
				var deferred = $q.defer();
				Message.loading();
				resource.save({ op: 'applySMan' }, function(response) {
					console.log(response);
					Message.hidden();
					if(response.code == 0) {
						deferred.resolve(response.data);
						...
						Message.show(response.msg);
						$state.go('sMan.center', { spid: $rootScope.globalInfo.user.isShop });
						var shopApply = Storage.get('user');
						shopApply.isShop = response.data;
						Storage.set('user', shopApply);
						$rootScope.globalInfo.user = shopApply;
			...
			//申请业务员，检查市级代理人是否存在
			checkCAgent: function(cAgentNum) {
				var deferred = $q.defer();
				Message.loading();
				resource.save({ op: 'applySMan', type: 'check', cAgentNum: cAgentNum }, function(response) {
					if(response.code == 0) {
						deferred.resolve(response.data);
					} else if(response.code == 1) {
						Message.show(response.msg);
			...
			//申请业务员
			applySMan: function(cAgentNum) {
				var deferred = $q.defer();
				Message.loading();
				resource.save({ op: 'applySMan', type: 'save', cAgentNum: cAgentNum }, function(response) {
					if(response.code == 301) {
						Message.show(response.msg, 2000);
						deferred.resolve(response);
					} else if(response.code == 1) {
			...
			checkshopPer: function(shopPer) {
				var deferred = $q.defer();
				Message.loading();
				resource.save({ op: 'applyfor', type: 'check', shopPer: shopPer }, function(response) {
					if(response.code == 0) {
						console.log(response)
						deferred.resolve(response.data.shopPer);
					} else if(response.code == 1) {
						Message.show(response.msg)
						return false;
					}
